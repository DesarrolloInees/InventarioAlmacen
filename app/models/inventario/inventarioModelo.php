...
class inventarioModelo
{
    ...
    /**
     * Establece la conexión con la base de datos remota (inventario-almacen).
     * Si falla por timeout o credenciales, realiza un fallback a la base local para no tumbar la app.
     */
    ...
    public function obtenerInventarioDisponible()
    {
        ...
        $sql = "SELECT 
                    ...
                    r.id_repuesto AS repuesto_id,
                    NULL AS producto_id,
                    'Repuesto' AS tipo_item,
     ...
     */
    public function actualizarInventario($inventario_id, $cantidad, $ubicacion, $estado, $tipoItem = 'Repuesto')
    {
        $conexionActiva = $this->obtenerConexionRemota();

        try {
            if ($tipoItem === 'Producto') {
                $sql = "INSERT INTO inventario_stock (id_producto, cantidad_total) 
                        VALUES (:id, :cantidad) 
                        ON DUPLICATE KEY UPDATE cantidad_total = :cantidad2";
            } else {
                $sql = "INSERT INTO inventario_stock (id_repuesto, cantidad_total) 
                        VALUES (:id, :cantidad) 
                        ON DUPLICATE KEY UPDATE cantidad_total = :cantidad2";
            }
    ...
    public function obtenerTecnicos()
    {
        ...
        // 2. Técnicos Motorizados (Conexión remota)
        try {
            $connMotos = $this->obtenerConexionRemota();
