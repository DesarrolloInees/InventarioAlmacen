<?php
// app/views/producto/productoVerVista.php
...
<style>
    ...
    .dark #prodTable tbody tr {
        background-color: #1f2937 !important;
        color: #d1d5db !important;
    }

    .dark #prodTable tbody tr:hover {
        background-color: #374151 !important;
    }
    ...
    <div
        ...
        <div
            ...
            <div>
                <h1 class="text-2xl sm:text-3xl font-bold text-gray-800 dark:text-white">
                    <i class="fas fa-soap text-purple-600 dark:text-purple-500 mr-2"></i> Productos (Consumibles)
                </h1>
                <p class="text-gray-500 dark:text-gray-400 mt-1">Catálogo de insumos de aseo, oficina y uso general.</p>
            </div>
            <div class="mt-4 sm:mt-0 flex flex-wrap gap-2">
                ...
                <?php if ($esAdmin): ?>
                    <a href="<?= BASE_URL ?>productoCrear"
                        class="px-5 py-2.5 bg-purple-600 text-white font-bold rounded-lg shadow-md hover:bg-purple-700 transition-transform transform hover:scale-105 flex items-center space-x-2">
        ...
        <?php if (!empty($data['productos'])): ?>
            <div class="overflow-x-auto">
                <table id="prodTable" class="w-full text-sm text-left text-gray-600 dark:text-gray-300">
                    <thead
                        class="text-xs text-gray-700 dark:text-gray-300 uppercase bg-gray-50 dark:bg-gray-700 border-b dark:border-gray-600">
                        <tr>
                            <th class="py-3 px-4">Cod. Interno</th>
                            <th class="py-3 px-4">Nombre de Producto</th>
                            <th class="py-3 px-4 text-right">Valor Venta (Ref)</th>
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                        <?php foreach ($data['productos'] as $p): ?>
                            <tr class="transition-colors border-b dark:border-gray-700">
                                <td class="py-4 px-4 font-mono font-bold text-gray-500 dark:text-gray-400">
                                    <?= htmlspecialchars($p['codigo_interno'] ?: 'S/C') ?>
                                </td>
                                <td class="py-4 px-4 font-bold text-gray-900 dark:text-white">
                                    <?= htmlspecialchars($p['nombre_producto']) ?>
                                </td>
                                <td class="py-4 px-4 text-right font-mono text-purple-600 dark:text-purple-400 font-medium">
                                    $ <?= number_format($p['valor_venta'], 2) ?>
                                </td>
                                <td class="py-4 px-4 text-center">
                                    <?php if ($p['estado'] == 1): ?>
                                        <span
                                            class="px-2 py-1 text-[10px] font-bold text-green-700 bg-green-100 dark:bg-green-900/30 rounded-full border border-green-200 dark:border-green-800 uppercase tracking-wider">Activo</span>
                                    <?php else: ?>
                                        <span
                                            class="px-2 py-1 text-[10px] font-bold text-red-700 bg-red-100 dark:bg-red-900/30 rounded-full border border-red-200 dark:border-red-800 uppercase tracking-wider">Inactivo</span>
                                    <?php endif; ?>
                                </td>
                                <?php if ($esAdmin): ?>
                                    <td class="py-4 px-4 text-center whitespace-nowrap">
                                        <div class="flex justify-center items-center space-x-2">
                                            <a href="<?= BASE_URL ?>productoEditar/<?= htmlspecialchars($p['id_producto']) ?>"
                                            ...
                                            <button onclick="abrirModalEliminar(<?= $p['id_producto'] ?>)"
        ...
        <?php else: ?>
            <div
                ...
                <i class="fas fa-box-open text-4xl text-purple-300 dark:text-purple-900/50 mb-3"></i>
                <p class="text-gray-500 dark:text-gray-400 font-medium">Aún no tienes productos registrados en el catálogo.
                </p>
            </div>
        <?php endif; ?>
    </div>
            ...
            <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white">Desactivar Producto</h3>
                        <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">¿Estás seguro de que deseas desactivar
                            este consumible? Dejará de estar disponible para compras e inventarios.</p>
                    </div>
                </div>
            </div>
            <div
                ...
                <a id="btnConfirmarEliminar" href="#"
                    class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-white hover:bg-red-700 sm:ml-3 sm:w-auto sm:text-sm">Sí,
                    Desactivar</a>
                <button type="button" onclick="cerrarModalEliminar()"
                    class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 dark:border-gray-600 shadow-sm px-4 py-2 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 hover:bg-gray-50 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">Cancelar</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    const listaProd = <?= json_encode($data['productos'] ?? []) ?>;

    function exportarExcel() {
        if (listaProd.length === 0) return alert("No hay datos");
        const datosExcel = listaProd.map(p => ({
            "Código": p.codigo_interno,
            "Nombre Producto": p.nombre_producto,
            "Valor Unitario": p.valor_venta,
    ...
    function abrirModalEliminar(id) {
        document.getElementById('btnConfirmarEliminar').href = "<?= BASE_URL ?>productoEliminar/" + id;
