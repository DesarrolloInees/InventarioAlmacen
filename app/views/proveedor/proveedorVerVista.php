<?php
// app/views/proveedor/proveedorVerVista.php
...
<style>
    ...
    .dark #provTable tbody tr {
        background-color: #1f2937 !important;
        color: #d1d5db !important;
    }

    .dark #provTable tbody tr:hover {
        background-color: #374151 !important;
    }
    ...
    <div
        ...
        <div
            ...
            <div>
                <h1 class="text-2xl sm:text-3xl font-bold text-gray-800 dark:text-white">
                    <i class="fas fa-truck-loading text-blue-600 dark:text-blue-500 mr-2"></i> Proveedores
                </h1>
                <p class="text-gray-500 dark:text-gray-400 mt-1">Directorio de suministros y compras de repuestos.</p>
            </div>
            <div class="mt-4 sm:mt-0 flex flex-wrap gap-2">
                ...
                <?php endif; ?>
                <?php if ($esAdmin): ?>
                    <a href="<?= BASE_URL ?>proveedorCrear"
                        ...
                        <i class="fas fa-plus-circle"></i> <span>Nuevo Proveedor</span>
                    </a>
                <?php endif; ?>
            </div>
        </div>
        ...
        <?php if (!empty($data['proveedores'])): ?>
            <div class="overflow-x-auto">
                <table id="provTable" class="w-full text-sm text-left text-gray-600 dark:text-gray-300">
                    <thead
                        class="text-xs text-gray-700 dark:text-gray-300 uppercase bg-gray-50 dark:bg-gray-700 border-b dark:border-gray-600">
                        <tr>
                            <th class="py-3 px-4">Empresa / Nombre</th>
                            <th class="py-3 px-4">NIT / Doc</th>
                        ...
                        <?php foreach ($data['proveedores'] as $p): ?>
                            <tr class="transition-colors border-b dark:border-gray-700">
                                <td class="py-4 px-4 font-bold text-gray-900 dark:text-white">
                                    <?= htmlspecialchars($p['nombre_proveedor']) ?></td>
                                <td class="py-4 px-4 font-mono"><?= htmlspecialchars($p['nit_documento'] ?: 'N/A') ?></td>
                                <td class="py-4 px-4">
                                    <div class="flex flex-col space-y-1">
                                        <?php if ($p['telefono']): ?><span class="text-xs"><i
                                                    class="fas fa-phone mr-1 text-gray-400"></i><?= htmlspecialchars($p['telefono']) ?></span><?php endif; ?>
                                        <?php if ($p['email']): ?><span class="text-xs"><i
                                                    class="fas fa-envelope mr-1 text-gray-400"></i><?= htmlspecialchars($p['email']) ?></span><?php endif; ?>
                                    </div>
                                </td>
                                ...
                                <?php if ($esAdmin): ?>
                                    <td class="py-4 px-4 text-center whitespace-nowrap">
                                        <div class="flex justify-center items-center space-x-2">
                                            <a href="<?= BASE_URL ?>proveedorEditar/<?= htmlspecialchars($p['id_proveedor']) ?>"
                                            ...
                                            <button onclick="abrirModalEliminar(<?= $p['id_proveedor'] ?>)"
        ...
        <?php else: ?>
            <div
                ...
                <i class="fas fa-truck text-4xl text-gray-300 dark:text-gray-600 mb-3"></i>
            ...
            <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white">Desactivar Proveedor
                        </h3>
                        <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">¿Estás seguro de que deseas desactivar
                            este proveedor? Su estado cambiará a "Inactivo".</p>
                    </div>
                </div>
            </div>
            <div
...
<script>
    const listProv = <?= json_encode($data['proveedores'] ?? []) ?>;
    function exportarExcel() {
        if (listProv.length === 0) return alert("No hay datos");
        const datosExcel = listProv.map(p => ({
            "Nombre": p.nombre_proveedor, "NIT/Doc": p.nit_documento, "Teléfono": p.telefono, "Email": p.email, "Estado": p.estado.toUpperCase()
        }));
        const ws = XLSX.utils.json_to_sheet(datosExcel);
        const wb = XLSX.utils.book_new();
        ws['!cols'] = [{ wch: 35 }, { wch: 15 }, { wch: 15 }, { wch: 25 }, { wch: 10 }];
        XLSX.utils.book_append_sheet(wb, ws, "Proveedores");
        XLSX.writeFile(wb, `Proveedores_${new Date().toISOString().slice(0, 10)}.xlsx`);
    }
    $(document).ready(function () { $('#provTable').DataTable({ responsive: true, language: { url: '//cdn.datatables.net/plug-ins/1.13.4/i18n/es-ES.json' }, dom: '<"flex flex-wrap justify-between items-center mb-4"lf>rt<"flex flex-wrap justify-between items-center mt-4"ip>' }); });
    function abrirModalEliminar(id) { document.getElementById('btnConfirmarEliminar').href = "<?= BASE_URL ?>proveedorEliminar/" + id; document.getElementById('modalEliminar').classList.remove('hidden'); }
