<?php
// app/views/categoria/categoriaVerVista.php
...
<style>
    ...
    .dark #catTable tbody tr {
        background-color: #1f2937 !important;
        color: #d1d5db !important;
    }

    .dark #catTable tbody tr:hover {
        background-color: #374151 !important;
    }
    ...
    <div
        ...
        <div
            ...
            <div>
                <h1 class="text-2xl sm:text-3xl font-bold text-gray-800 dark:text-white">
                    <i class="fas fa-layer-group text-orange-500 mr-2"></i> Categorías del Sistema
                </h1>
                <p class="text-gray-500 dark:text-gray-400 mt-1">Clasificación general para repuestos y consumibles.</p>
            </div>
            <div class="mt-4 sm:mt-0 flex flex-wrap gap-2">
                ...
                <?php if ($esAdmin): ?>
                    <a href="<?= BASE_URL ?>categoriaCrear"
                        ...
                        <i class="fas fa-plus-circle"></i> <span>Nueva Categoría</span>
                    </a>
                <?php endif; ?>
            </div>
        </div>
        ...
        <?php if (!empty($data['categorias'])): ?>
            <div class="overflow-x-auto">
                <table id="catTable" class="w-full text-sm text-left text-gray-600 dark:text-gray-300">
                    <thead
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                        <?php foreach ($data['categorias'] as $c): ?>
                            <tr class="transition-colors border-b dark:border-gray-700">
                                <!-- Corrección de columna: id_categoria -->
                                <td class="py-4 px-4 font-bold text-gray-500">#<?= htmlspecialchars($c['id_categoria']) ?></td>

                                <!-- Corrección de columna: nombre_categoria -->
                                <td class="py-4 px-4 font-bold text-gray-900 dark:text-white">
                                    <?= htmlspecialchars($c['nombre_categoria']) ?></td>

                                <td class="py-4 px-4 text-gray-500 dark:text-gray-400">
                                    <?= htmlspecialchars($c['descripcion'] ?: 'Sin descripción') ?></td>

                                <td class="py-4 px-4 text-center">
                                    <!-- Corrección lógica del estado (1 o 0) -->
                                    <?php if ($c['estado'] == 1): ?>
                                        <span
                                ...
                                <?php if ($esAdmin): ?>
                                    <td class="py-4 px-4 text-center whitespace-nowrap">
                                        <div class="flex justify-center items-center space-x-2">
                                            <a href="<?= BASE_URL ?>categoriaEditar/<?= htmlspecialchars($c['id_categoria']) ?>"
                                            ...
                                            <button onclick="abrirModalEliminar(<?= $c['id_categoria'] ?>)"
        ...
        <div
            ...
            <div class="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div class="sm:flex sm:items-start">
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        <h3 class="text-lg leading-6 font-medium text-gray-900 dark:text-white">Desactivar Categoría
                        </h3>
            ...
            <div
                ...
                <button type="button" onclick="cerrarModalEliminar()"
                    class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 dark:border-gray-600 shadow-sm px-4 py-2 bg-white dark:bg-gray-800 text-gray-700 hover:bg-gray-50 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">Cancelar</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    const listaCat = <?= json_encode($data['categorias'] ?? []) ?>;

    function exportarExcel() {
        if (listaCat.length === 0) return alert("No hay datos");
        const datosExcel = listaCat.map(c => ({
            "ID": c.id_categoria, // Corrección export
            "Nombre": c.nombre_categoria, // Corrección export
            "Descripción": c.descripcion,
            "Estado": (c.estado == 1) ? 'ACTIVO' : 'INACTIVO' // Corrección export
        }));
    ...
    function abrirModalEliminar(id) {
        document.getElementById('btnConfirmarEliminar').href = "<?= BASE_URL ?>categoriaEliminar/" + id;
