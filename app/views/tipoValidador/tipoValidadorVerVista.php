...
<style>
    ...
    #validadoresTable tbody tr {
        background-color: white !important;
    }

    #validadoresTable tbody tr:hover {
    ...
    .dark #validadoresTable tbody tr {
        background-color: #1f2937 !important;
        color: #d1d5db !important;
    }

    .dark #validadoresTable tbody tr:hover {
        background-color: #374151 !important;
    }
    ...
    <div
        ...
        <div
            ...
            <div>
                <h1 class="text-2xl sm:text-3xl font-bold text-gray-800 dark:text-white">
                    <i class="fas fa-credit-card text-blue-600 dark:text-blue-500 mr-2"></i> Tipos de Validador
                </h1>
                <p class="text-gray-500 dark:text-gray-400 mt-1">Gestiona los equipos validadores del sistema.</p>
            </div>
            <div class="mt-4 sm:mt-0 flex flex-wrap gap-2">
                <?php if (!empty($data['validadores'])): ?>
                ...
                <?php endif; ?>
                <?php if ($esAdmin): ?>
                    <a href="<?= BASE_URL ?>tipoValidadorCrear"
                        ...
                        <i class="fas fa-plus-circle"></i> <span>Nuevo Validador</span>
                    </a>
                <?php endif; ?>
            </div>
        </div>
        ...
        <?php if (!empty($data['validadores'])): ?>
            <div class="overflow-x-auto">
                <table id="validadoresTable" class="w-full text-sm text-left text-gray-600 dark:text-gray-300">
                    <thead
                        class="text-xs text-gray-700 dark:text-gray-300 uppercase bg-gray-50 dark:bg-gray-700 border-b dark:border-gray-600">
                        <tr>
                            <th class="py-3 px-4">ID</th>
                            <th class="py-3 px-4">Tipo de Validador</th>
                                ...
                                <?php if ($esAdmin): ?>
                                    <td class="py-4 px-4 text-center whitespace-nowrap">
                                        <div class="flex justify-center items-center space-x-2">
                                            <a href="<?= BASE_URL ?>tipoValidadorEditar/<?= htmlspecialchars($v['idTipoValidador']) ?>"
                                            ...
                                            <button onclick="abrirModalEliminar(<?= $v['idTipoValidador'] ?>)"
        ...
        <?php else: ?>
            <div
                ...
                <p class="text-gray-500 dark:text-gray-400 font-medium">No hay tipos de validador registrados.</p>
            </div>
        <?php endif; ?>
    </div>
</div>
                    ...
                    <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        ...
                        <div class="mt-2">
                            <p class="text-sm text-gray-500 dark:text-gray-400">
                                ¿Estás seguro de que deseas desactivar este validador? Su estado cambiará a "Inactivo"
                                en el sistema.
                            </p>
                        </div>
                    </div>
...
<script>
    const listaValidadores = <?= json_encode($data['validadores'] ?? []) ?>;
    function exportarExcel() {
        if (listaValidadores.length === 0) return alert("No hay datos");
        const datosExcel = listaValidadores.map(v => ({
            "ID": v.idTipoValidador, "Nombre Validador": v.nombreTipoValidador, "Estado": v.estado.charAt(0).toUpperCase() + v.estado.slice(1)
        }));
        const ws = XLSX.utils.json_to_sheet(datosExcel);
        const wb = XLSX.utils.book_new();
        ws['!cols'] = [{ wch: 10 }, { wch: 40 }, { wch: 15 }];
        XLSX.utils.book_append_sheet(wb, ws, "Validadores");
        XLSX.writeFile(wb, `Tipos_Validador_${new Date().toISOString().slice(0, 10)}.xlsx`);
    }

    $(document).ready(function () {
        $('#validadoresTable').DataTable({ responsive: true, language: { url: '//cdn.datatables.net/plug-ins/1.13.4/i18n/es-ES.json' }, dom: '<"flex flex-wrap justify-between items-center mb-4"lf>rt<"flex flex-wrap justify-between items-center mt-4"ip>' });
    });

    // Lógica del Modal de Eliminación
    function abrirModalEliminar(id) {
        const urlBase = "<?= BASE_URL ?>";
        document.getElementById('btnConfirmarEliminar').href = urlBase + "tipoValidadorEliminar/" + id;
